...
const { fullySortLastBag } = require('./lastBagSorting');
...
async function main(){
    ...
    await logging(`👜 ${accountsInLastBag.length} accounts in last bag`);
    await logging("----------------------------------");
    ...
    // Get the accounts that need repositioning and the last bag fully sorted based on stake
    let { putInFrontOf, sortedBag } = await fullySortLastBag(accountsInLastBag);

    await logging("🔢 Sorted bag");
    await logging("-------------");
    // Log the accounts in the sorted bag and their index
    for (i = 0; i < sortedBag.length; i++) {
        await logging(i);
        await logging(sortedBag[i]);
    }
    ...
    if (putInFrontOf.length > 0) {
        ...
        putInFrontOf = fullySortLastBag(adjustedBag);
        ...
        // Finally we do an automated sanity check that no accounts with higher score are below any accounts with lower score
        for (let i = 0; i < adjustedBag.length - 1; i++) {
            if (adjustedBag[i].score < adjustedBag[i+1].score) {
                await logging(`Lighter account ${adjustedBag[i].id} with score ${adjustedBag[i].score} at index ${i} is in front of heavier account ${adjustedBag[i+1].id} with score ${adjustedBag[i+1].score}`);
            }
        }
